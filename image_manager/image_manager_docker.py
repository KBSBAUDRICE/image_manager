...
# Plugin Modules
from .image_manager_calculations import *
from .image_manager_extension import imagemanager_Extension
from .image_manager_modulo import (
    imagemanager_Preview,
    imagemanager_Grid,
    imagemanager_Reference,
    Picker_Block,
    Picker_Color_HUE,
    Picker_Color_HSV,
    Function_Process,
...
# Plugin
DOCKER_NAME = "Image Manager"
image_manager_version = "2025_09_07"
...
class imagemanager_Docker( DockWidget ):

    #region Initialize

    def __init__( self ):
        super( imagemanager_Docker, self ).__init__()
    ...
    def User_Interface( self ):
        ...
        # Widget Docker
        self.layout = uic.loadUi( os.path.join( self.directory_plugin, "image_manager_docker.ui" ), QWidget( self ) )
        self.setWidget( self.layout )

        # Settings
        self.dialog = uic.loadUi( os.path.join( self.directory_plugin, "image_manager_settings.ui" ), QDialog( self ) )
        self.dialog.setWindowTitle( "Image Manager : Settings" )
        ...
        self.picker = uic.loadUi( os.path.join( self.directory_plugin, "image_manager_picker.ui" ), QWidget( self ) )
        self.picker.setParent( self )
        self.picker.close()
    def Variables( self ):
        # Pykrita
        self.imagine_pyid = "pykrita_image_manager_docker"
        ...
        self.scale_method = True
        self.inverse_direction = True
        ...
        self.slideshow_lottery = [0]
        # Preview Display
        self.preview_display = False

        # Grid
        self.grid_automatic = True
        self.grid_size = 200
        self.grid_x = 3
        self.grid_y = 3
        self.grid_fit = False
    ...
    def Connections( self ):
        ...
        #tag
        self.layout.directory.itemSelectionChanged.connect(self.Item_Changed)
        self.layout.directory.itemChanged.connect(self.SaveItem)
        self.layout.directory.customContextMenuRequested.connect(self.Context_Menu_Item)
        ...
        self.dialog.inverse_direction.toggled.connect( self.Inverse_Direction )
        self.dialog.background_color.clicked.connect( self.Background_Color )

        #tag
        self.dialog.automatic_tag.toggled.connect(self.Automatic_tag)
        self.dialog.height_tag.valueChanged.connect(self.Height_Tag)
        self.dialog.prefix.textChanged.connect(self.Prefix)
        self.dialog.suffix.textChanged.connect(self.Suffix)
        ...
        # Dialog Display Grid
        self.dialog.grid_automatic.toggled.connect(self.Menu_Grid_Automatic)
        self.dialog.grid_x.valueChanged.connect( self.Menu_Grid_X )
        self.dialog.grid_y.valueChanged.connect( self.Menu_Grid_Y )
    ...
    def Modules( self ):
        ...
        self.imagine_preview = imagemanager_Preview( self.layout.preview_view )
        ...
        self.imagine_grid = imagemanager_Grid( self.layout.imagine_grid )
        ...
        self.imagine_reference = imagemanager_Reference( self.layout.reference_view )
    ...
    def Style( self ):
        ...
        #self.layout.directory.setStyleSheet("QScrollBar:horizontal{ width: 20px;}")
        # Function Operations
    ...
    def Extension( self ):
        ...
        extension = imagemanager_Extension( parent = Krita.instance() )
    ...
    def Settings( self ):
        ...
        self.inverse_direction = self.Set_Read( "EVAL", "inverse_direction", self.inverse_direction )
        self.background_color.setNamedColor(self.Set_Read( "STR", "background_color", self.background_color.name() ))
        #tag
        self.automatic_tag = self.Set_Read( "EVAL", "automatic_tag", self.automatic_tag )
        self.height_tag = self.Set_Read( "INT", "height_tag", self.height_tag )
        self.prefix = self.Set_Read( "STR", "prefix", self.prefix )
        self.suffix = self.Set_Read( "STR", "suffix", self.suffix )
        # Preview
        ...
        # Grid
        self.grid_automatic = self.Set_Read("EVAL", "grid_automatic", self.grid_automatic)
        self.grid_x = self.Set_Read( "INT", "grid_x", self.grid_x )
        self.grid_y = self.Set_Read( "INT", "grid_y", self.grid_y )
    ...
    def Delete_Item(self):
        i = self.layout.directory.currentRow()
        if( i > -1):
            self.layout.directory.takeItem(i)
            self.SaveItem()
            self.Save_Preview_List()
    
    def Delete_Item_Right(self):
        i = self.layout.directory.currentRow()
        if( i > -1 ):
            self.layout.directory.itemSelectionChanged.disconnect(self.Item_Changed)
            self.layout.directory.itemChanged.disconnect(self.SaveItem)
            if( i == 0 and self.layout.directory.count() == 1):
                self.layout.directory.takeItem(0)
                self.layout.directory.setCurrentRow(-1)
            else:
                while i < self.layout.directory.count(): 
                    self.layout.directory.takeItem(self.layout.directory.count() - 1)
            self.SaveItem()
            self.Save_Preview_List()
            self.layout.directory.itemSelectionChanged.connect(self.Item_Changed)
            self.layout.directory.itemChanged.connect(self.SaveItem)
    ...
    def Delete_All_Item(self):
        if(self.layout.directory.count() > 0):            
            self.layout.directory.itemSelectionChanged.disconnect(self.Item_Changed)
            self.layout.directory.itemChanged.disconnect(self.SaveItem)
            self.layout.directory.setCurrentRow(-1)
            while 0 < self.layout.directory.count():
                self.layout.directory.takeItem(0)
            self.SaveItem()
            self.Save_Preview_List()
            self.layout.directory.itemSelectionChanged.connect(self.Item_Changed)
    ...
    def Change_Folder(self):
        ...
        if folder_path not in [ "", ".", None ]:
            ...
            self.layout.directory.currentItem().setToolTip(folder_path)
            
    def Context_Menu_Item(self, even):
        menu = QMenu(self)
        menu.setToolTipsVisible(True)
        add = QAction("Add Tab")
        open_path = QAction("Open Folder")
        rename = QAction("Rename")
        change_folder = QAction("Change The Folder")
        delete_menu = QAction("Close")
        delete_right = QAction("Close +")
        delete_right.setToolTip("Close this tab and everything after")
        delete_all_menu = QAction("Close All")
        menu.addAction(add)
        if(self.layout.directory.itemAt(even) != None):
            menu.addAction(open_path)
            menu.addAction(rename)
            menu.addAction(change_folder)
            menu.addAction(delete_menu)
            menu.addAction(delete_right)
        if(self.layout.directory.count() > 0): 
            menu.addAction(delete_all_menu)
        action = menu.exec_( self.layout.directory.mapToGlobal( even ) )
        if action == add:
            self.Folder_Open()
        if action == open_path:
            self.File_Location(self.layout.directory.currentItem().toolTip(), True)
        if action == rename:
            self.layout.directory.editItem(self.layout.directory.currentItem())
        if action == change_folder:
            self.Change_Folder()
        if action == delete_menu:
            self.Delete_Item()
        if action == delete_right:
            self.Delete_Item_Right()
        if action == delete_all_menu:
            self.Delete_All_Item()
    
    def Setup_Item_List(self):
        text = Krita.instance().readSetting( "Image Manager", "directory_text", "" )
        path = Krita.instance().readSetting( "Image Manager", "directory_path", "" )
        index = Krita.instance().readSetting( "Image Manager", "preview_list",  "" )
        if(text != "" and path != ""):
            text_list = text.split("#@$*#")
            path_list = path.split("#@$*#")
            index_list = index.split("#@$*#")
            i = 0
            if(len(text_list) == len(path_list)):
                while i < len(text_list):
                    if(path_list[i] !=  ""):
                        item = QListWidgetItem(text_list[i], self.layout.directory)
                        item.setToolTip(path_list[i])
                        item.setStatusTip(index_list[i])
                        item.setFlags(item.flags() | Qt.ItemIsEditable)
                        self.layout.directory.addItem(item)
                    i += 1
            if(self.layout.directory.count() > 0):
                if(self.folder_path == ""):
                    self.layout.directory.setCurrentRow(0)
                else:
                    i = 0
                    while i <  self.layout.directory.count():
                        if(self.layout.directory.item(i).toolTip() == self.folder_path):
                            self.layout.directory.setCurrentRow(i)
                            break
                        i += 1
    ...
    def Loader( self ):
        ...
        #inverse scroll
        self.dialog.inverse_direction.setChecked(self.inverse_direction)
        self.Inverse_Direction(self.inverse_direction)

        #background image        
        colorImage = QPixmap(32,32)
        colorImage.fill(self.background_color)
        colorIcon = QIcon(colorImage)
        
        self.dialog.background_color.setIcon(colorIcon)
        self.layout.stacked_widget.setStyleSheet(" #stacked_widget { background-color: " + self.background_color.name() + "; color: black}")

        #tag
        self.dialog.automatic_tag.setChecked(self.automatic_tag)
        self.dialog.height_tag.setValue(self.height_tag)
        self.dialog.prefix.setText(self.prefix)
        self.dialog.suffix.setText(self.suffix)
        ...
        # Grid
        self.dialog.grid_automatic.setChecked(self.grid_automatic)
        self.Menu_Grid_Automatic(self.grid_automatic)
        self.dialog.grid_x.setValue( self.grid_x )
        self.dialog.grid_y.setValue( self.grid_y )
    ...
    def Set_Read( self, mode, entry, default ):
        setting = Krita.instance().readSetting( "Image Manager", entry, "" )
        if setting == "":
            read = default
        else:
            try:
                ...
                read = default
        Krita.instance().writeSetting( "Image Manager", entry, str( read ) )
        return read
    ...
    def Mode_Index( self, index ):
        ...
        if self.mode_index != index:
            ...
            self.Update_Size()
        # Save
        Krita.instance().writeSetting( "Image Manager", "mode_index", str( self.mode_index ) )
    ...
    def Screen_Maximized( self, boolean ):
        if boolean == True:
            ...
            plugin = "pykrita_image_manager_docker"
    ...
    def Sync_List( self, sync_list ):
        ...
        if self.state_load == True:
            self.Filter_Search()
        # Finish
        Krita.instance().writeSetting( "Image Manager", "sync_list", str( self.sync_list ) )
        # self.update()
    def Sync_Type( self, sync_type ):
        ...
        if self.state_load == True:
            self.Filter_Search()
        # Save
        Krita.instance().writeSetting( "Image Manager", "sync_type", str( self.sync_type ) )
    def Sync_Sort( self, sync_sort ):
        ...
        if self.state_load == True:
            self.Filter_Search()
        # Save
        Krita.instance().writeSetting( "Image Manager", "sync_sort", str( self.sync_sort ) )
    def Search_Recursive( self, search_recursive ):
        self.search_recursive = search_recursive
        if self.state_load == True:
            self.Filter_Search()
        Krita.instance().writeSetting( "Image Manager", "search_recursive", str( self.search_recursive ) )
    def Insert_Size( self, insert_size ):
        self.insert_size = insert_size
        Krita.instance().writeSetting( "Image Manager", "insert_size", str( self.insert_size ) )
    def Scale_Method( self, scale_method ):
        ...
        Krita.instance().writeSetting( "Image Manager", "scale_method", str( self.scale_method ) )
    
    def Inverse_Direction(self, value):
        self.inverse_direction = value
        Krita.instance().writeSetting( "Image Manager", "inverse_direction", str( self.inverse_direction ) )
        self.layout.index_slider.setInvertedControls(value)
        if(value):
            self.imagine_preview.inverse_direction = -1
            self.imagine_grid.inverse_direction = -1
        else:
            self.imagine_preview.inverse_direction = 1
            self.imagine_grid.inverse_direction = 1

    def Background_Color(self):
        self.background_color = QColorDialog.getColor(self.background_color, self.dialog, "Background Color")
        colorImage = QPixmap(32,32)
        colorImage.fill(self.background_color)
        colorIcon = QIcon(colorImage)
        self.dialog.background_color.setIcon(colorIcon)
        self.layout.stacked_widget.setStyleSheet(" #stacked_widget { background-color: " + self.background_color.name() + "; color: black}")
        Krita.instance().writeSetting( "Image Manager", "background_color", self.background_color.name() )

    #tag
    def Automatic_tag(self, value):
        self.automatic_tag = value
        Krita.instance().writeSetting( "Image Manager", "automatic_tag", str( self.automatic_tag ) )

    def Height_Tag(self, value):
        self.height_tag = value
        size = QSize(self.layout.directory.maximumWidth(), value)
        self.layout.directory.setMaximumSize(size)
        Krita.instance().writeSetting( "Image Manager", "height_tag", str( self.height_tag ) )
        
    def Prefix(self, value):
        self.prefix = value
        Krita.instance().writeSetting( "Image Manager", "prefix", self.prefix )

    def Suffix(self, value):
        self.suffix = value
        Krita.instance().writeSetting( "Image Manager", "suffix", self.suffix)

    # Preview
    def Menu_SlideShow_Sequence( self, slideshow_sequence ):
        self.slideshow_sequence = slideshow_sequence
        Krita.instance().writeSetting( "Image Manager", "slideshow_sequence", str( self.slideshow_sequence ) )
    def Menu_SlideShow_Time( self, qtime ):
        self.slideshow_time = QTime( 0, 0, 0 ).msecsTo( qtime )
        Krita.instance().writeSetting( "Image Manager", "slideshow_time", str( self.slideshow_time ) )
    def Preview_Display( self, boolean ):
        self.preview_display = boolean
        self.imagine_preview.Set_Display( self.preview_display )
        Krita.instance().writeSetting( "Image Manager", "preview_display", str( self.preview_display ) )
    # Grid
    def Update_Grid_Size(self):
        self.imagine_grid.Grid_Size( self.grid_size, self.grid_automatic,  self.grid_x ,  self.grid_y)
        self.Display_Update()

    def Menu_Grid_Automatic(self, value):
        if(value):
            self.dialog.grid_size.setEnabled(True)
            self.dialog.grid_x.setEnabled(False)
            self.dialog.grid_y.setEnabled(False)
        else:
            self.dialog.grid_size.setEnabled(False)
            self.dialog.grid_x.setEnabled(True)
            self.dialog.grid_y.setEnabled(True)
        self.grid_automatic = value
        self.Update_Grid_Size()
        Krita.instance().writeSetting( "Image Manager", "grid_automatic", str( self.grid_automatic ) )

    def Menu_Grid_X(self, value):
        self.grid_x = value
        self.Update_Grid_Size()
        Krita.instance().writeSetting( "Image Manager", "grid_x", str( self.grid_x ) )

    def Menu_Grid_Y(self, value):
        self.grid_y = value
        self.Update_Grid_Size()
        Krita.instance().writeSetting( "Image Manager", "grid_y", str( self.grid_y ) )

    def Menu_Grid_Size( self, value):
        self.grid_size = value
        self.Update_Grid_Size()
        Krita.instance().writeSetting( "Image Manager", "grid_size", str( self.grid_size ) )
    
    def Menu_Grid_Fit( self, boolean ):
        self.grid_fit = boolean
        self.imagine_grid.Grid_Fit( self.grid_fit )
        Krita.instance().writeSetting( "Image Manager", "grid_fit", str( self.grid_fit ) )
    # Reference
    def Menu_Ref_Import( self, ref_import ):
        self.ref_import = ref_import
        Krita.instance().writeSetting( "Image Manager", "ref_import", str( self.ref_import ) )
    ...
    def ShowOnWelcome_Imagine( self, sow_imagine ):
        ...
        Krita.instance().writeSetting( "Image Manager", "sow_imagine", str( self.sow_imagine ) )
    def ShowOnWelcome_Dockers( self, sow_dockers ):
        self.sow_dockers = sow_dockers
        Krita.instance().writeSetting( "Image Manager", "sow_dockers", str( self.sow_dockers ) )
    ...
    def Menu_Transparent( self, transparent ):
        ...
        Krita.instance().writeSetting( "Image Manager", "transparent", str( self.transparent ) )
    ...
    def Menu_Settings( self ):
        ...
        size_x = 500
        size_y = 600
        self.dialog.setGeometry( int( px + w2 - size_x * 0.5 ), int( py + h2 - size_y * 0.5 ), int( size_x ), int( size_y ) )
        # Updates
        self.Combobox_Code()
    ...
    def Message_Log( self, operation, message ):
        string = f"Image Manager | { operation } { message }"
    ...
    # File
    def File_Location( self, image_path, path = False ):
        if path:
        ...
        if len( lista ) > 0:
            ...
            if check_html == True:
                ...
                for item in lista:
                    ...
                    if check_dir == True:
                        self.Add_Item(item)
                        if(self.automatic_tag):
                            it = QDirIterator( item, QDir.Dirs | QDir.NoDotAndDotDot, QDirIterator.Subdirectories)
                            while( it.hasNext() ):
                                dir = QDir(it.next())
                                self.Add_Item(dir.canonicalPath())
                    # Open
    ...
    def Imagine_Sample_Script( self ):
        ...
        pyid = "pykrita_image_manager_docker"
        dockers = Krita.instance().dockers()
        for i in range( 0, len( dockers ) ):
            if dockers[i].objectName() == pyid:
                image_manager = dockers[i]
                break
        image_manager.API_Preview_QPixmap( qpixmap )
        """
    ...
    def Folder_Menu( self, event ):
        if self.folder_path == None:
            ...
            add_tab = qmenu.addAction( "Add Current Folder In Tab" )
            qmenu.addSection( " " )
            # Child Dirs
            actions = {}
            ...
            if action == cmenu_parent:
                ...
                self.Add_Item(self.folder_path)
            else:
                for i in range( 0, len( self.folder_shift ) ):
                    if action == actions[i]:
    ...
    def Folder_Open( self ):
        ...
        if folder_path not in [ "", ".", None ]:
            #tag
            self.Add_Item(folder_path)
            if(self.automatic_tag):
                it = QDirIterator( folder_path, QDir.Dirs | QDir.NoDotAndDotDot, QDirIterator.Subdirectories)
                while( it.hasNext() ):
                    dir = QDir(it.next())
                    self.Add_Item(dir.canonicalPath())
            self.Folder_Load( folder_path, 0)
    ...
    def Folder_Load( self, folder_path, index ):
        try:
            ...
            self.folder_path = ""
        # Save
        Krita.instance().writeSetting( "Image Manager", "folder_path", str( self.folder_path ) )

    # tag -preview_list
    def Save_Preview_List(self):
        text = ""
        if(self.layout.directory.count() > 0):           
            i = 0
            while i < self.layout.directory.count():
                text = text + "#@$*#" + self.layout.directory.item(i).statusTip()
                i += 1
        Krita.instance().writeSetting( "Image Manager", "preview_list",  text )

    def Get_Current_Preview_List(self):
        index = self.layout.directory.currentRow()
        if(index >= 0 ):
            return int(self.layout.directory.currentItem().statusTip())
        else:
            return 0
    
    def Set_Curent_Preview_List(self, value):        
        index = self.layout.directory.currentRow()
        if(index >= 0 ):
            self.layout.directory.currentItem().setStatusTip(str(value))               
            self.Save_Preview_List()
    ...
    def Item_Changed(self):
        index = self.layout.directory.currentRow()
        if(index >= 0 ):
            self.Folder_Load(self.layout.directory.currentItem().toolTip(), self.Get_Current_Preview_List())
        
    def Add_Item(self, path):
        dir = QDir(path)
        if dir.exists() and dir.dirName() not in [ "", ".", None ]:
            item = QListWidgetItem(self.prefix + dir.dirName() + self.suffix, self.layout.directory)
            item.setToolTip(dir.canonicalPath())
            item.setFlags(item.flags() | Qt.ItemIsEditable)
            item.setStatusTip(str(0))
            self.layout.directory.setCurrentRow(-1)
            self.layout.directory.addItem(item)
            self.Save_Preview_List()
            self.SaveItem()
    ...
    def SaveItem(self, value = ""):
        ...
        if(self.layout.directory.count() > 0):           
            i = 0
            while i < self.layout.directory.count():
                text = text + "#@$*#" + self.layout.directory.item(i).text()
                path = path + "#@$*#" + self.layout.directory.item(i).toolTip()
                i += 1
        Krita.instance().writeSetting( "Image Manager", "directory_text",  text )
        Krita.instance().writeSetting( "Image Manager", "directory_path",  path )
    

    def Filter_Search( self ):
        self.search = self.layout.search.text().lower()
        self.Filter_Keywords( self.search, 0 )
        Krita.instance().writeSetting( "Image Manager", "search", str( self.search ) )
    ...
    def Filter_Keywords( self, search, index ):
        try:
            ...
            if self.sync_list == "Folder":
                ...
                files = list()
                if(len(self.qdir.entryInfoList()) > 0):
                    files = self.qdir.entryInfoList()
                # Recursive Files
                if self.search_recursive == True:
                    it = QDirIterator( self.folder_path, QDir.Dirs | QDir.NoDotAndDotDot, QDirIterator.Subdirectories)
                    while( it.hasNext() ):
                        dir = QDir(it.next())
                        dir.setSorting( self.file_sort )
                        dir.setFilter( QDir.Files | QDir.NoSymLinks | QDir.NoDotAndDotDot )
                        dir.setNameFilters( self.file_extension )
                        if(len(dir.entryInfoList()) > 0):
    ...
    def Index_Slider( self, value ):
        self.Index_Block( True )
        self.preview_index = value - 1
        self.Set_Curent_Preview_List(value - 1) # Humans start at 1 and script starts at 0
    ...
    def Preview_Index( self, index ):
        if ( self.file_found == True and self.preview_index != index ):
            ...
            self.Set_Curent_Preview_List(Limit_Range( index, 0, self.preview_max ))
    ...
    def Reference_Locked( self, ref_locked ):
        ...
        # Save
        Krita.instance().writeSetting( "Image Manager", "ref_locked", str( self.ref_locked ) )
    ...
    def Link_KRA( self, boolean ):
        ...
        if boolean == True:
            ...
            self.Link_Live()
        Krita.instance().writeSetting( "Image Manager", "ref_kra", str( self.ref_kra ) )
    def Link_Live( self ):
    ...
    def Link_Load( self ):
        if ( self.canvas() is not None ) and ( self.canvas().view() is not None ):
            ...
            annotation = doc.annotation( "Image Manager" )
    ...
    def Link_Save( self ):
        if ( self.canvas() is not None ) and ( self.canvas().view() is not None ):
            data = self.Data_Save()
            doc = Krita.instance().activeDocument()
            doc.setAnnotation( 'Image Manager', "Document", QByteArray( data.encode() ) )
            doc.save()
    ...
    def Data_Load( self, board, ref_board ):
        ...
        if len( board ) > 0:
            if board[0].startswith( "Image Manager" ) == True:
    ...
    def Data_Kritarc( self ):
        ...
        Krita.instance().writeSetting( "Image Manager", "ref_board", str( self.ref_board ) )
    ...
    def Label_Text( self ):
        ...
        if previous != None:
            string, ok = QInputDialog.getMultiLineText( self, "Image Manager", "Input Text", previous["text"] )
    ...
    def Reference_Camera( self, ref_position, ref_zoom, len_board ):
        ...
        # Save
        Krita.instance().writeSetting( "Image Manager", "ref_position", str( self.ref_position ) )
        Krita.instance().writeSetting( "Image Manager", "ref_zoom", str( self.ref_zoom ) )
    ...
    def Function_String_List( self ):
        ...
        Krita.instance().writeSetting( "Image Manager", "function_string", str( self.function_string ) )
    ...
    def Watcher_State( self, boolean ):
        # Blocks Image Manager from updating to changes to the Directory while Function>> works
...
class Worker_Function( QObject ):
    ...
    def run( self, source, mode, path_source, path_destination, file_list, operation, number, keyword, python_script ):
        ...
        delta = start.msecsTo( end )
        time = QTime( 0,0 ).addMSecs( delta )
        try:QtCore.qDebug( f"Image Manager | FUNCTION>> { time.toString( 'hh:mm:ss.zzz' ) } | OPERATION { str( operation ).lower() }" )
        except:pass
            ...
            if path_new != None:
                ...
                if ( path != path_new and exists == False ):
                    if operation_type == "rename":
                        ...
                        if boolean == True:
                            try:QtCore.qDebug( f"Image Manager | RENAME { os.path.basename( path ) } >> { os.path.basename( path_new ) }" )
                            except:pass
                    if operation_type == "save":
                        ...
                        if boolean == True:
                            try:QtCore.qDebug( f"Image Manager | SAVE { path_new }" )
                            except:pass
                # Anchor Find
...
class Worker_Python( QObject ):
    ...
    def run( self, python_script, image_file, path_destination ):
        # Implicit Paths
        # image_file = path_source files or file collection, given by Image Manager
        # path_destination = directory path to save files into, given by Image Manager
        try:
            exec( python_script )
        except Exception as e:
            try:QtCore.qDebug( f"Image Manager | ERROR Python script\nimage_file={ image_file } path_destination={ path_destination }\n{ e }" )
            except:pass
