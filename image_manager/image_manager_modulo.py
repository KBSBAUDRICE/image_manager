...
# Cursor
def Cursor_Icon( self, execption = False ):
    if ( (self.state_pickcolor == True and self.state_press == True) or execption == True ):
...
def Path_Copy( self, path ):
    ...
    copy.setText( path )
# Color Picker
def ColorPicker_Event( self, ex, ey, qimage_grab, state_press, exeption = False ):
    if ( (self.state_pickcolor == True and qimage_grab != None) or exeption == True ):
        # Event
...
class imagemanager_Preview( QWidget ):
    # General
    ...
    def __init__( self, parent ):
        super( imagemanager_Preview, self ).__init__( parent )
        self.Variables()
    def Variables( self ):
        # Widget
        self.ww = 1
        ...
        self.color_alpha = QColor( 255, 255, 255, 0 )
    ...
    def Anim_Export_Index( self, index ):
        if self.state_animation == True:
            ...
            if exists == False:
                ...
                # Logger
                try:QtCore.qDebug( f"Image Manager | SCREENSHOT { index }:{ self.frame_count }" )
                except:pass
                # Garbage
    ...
    def Comp_Export_Index( self, index ):
        if self.state_compact == True:
            ...
            if exists == False and qpixmap.isNull() == False:
                ...
                string = f"Image Manager | ERROR zip file not saved"
    ...
    def mousePressEvent( self, event ):
        ...
        # Cursor
        execption =( event.modifiers() == QtCore.Qt.AltModifier and event.buttons() == QtCore.Qt.RightButton )
        Cursor_Icon( self, execption )
        ...
        if ( event.modifiers() == QtCore.Qt.AltModifier and event.buttons() == QtCore.Qt.RightButton ):
            self.operation = "color_picker"
            ColorPicker_Event( self, self.ex, self.ey, self.qimage_grab, True, True )
    ...
    def mouseMoveEvent( self, event ):
        ...
        # Neutral
        execption = ( event.modifiers() == QtCore.Qt.AltModifier and event.buttons() == QtCore.Qt.RightButton )
        if ( (self.operation == "color_picker" and self.anim_timer.isActive() == False) or execption ):
            ColorPicker_Event( self, self.ex, self.ey, self.qimage_grab, True, execption )
    ...
    def wheelEvent( self, event ):
        ...
        angle = 5
        if delta_y >= angle:
            self.SIGNAL_INCREMENT.emit( +1 * self.inverse_direction)
        if delta_y <= -angle:
            self.SIGNAL_INCREMENT.emit( -1 * self.inverse_direction)
    # Drag and Drop Event
...
class imagemanager_Grid( QWidget ):
    # General
    SIGNAL_DRAG = QtCore.pyqtSignal( [ str, dict ] )
    SIGNAL_DROP = QtCore.pyqtSignal( list )
    # Grid
    ...
    # Init
    def __init__( self, parent ):
        super( imagemanager_Grid, self ).__init__( parent )
        self.Variables()
    def Variables( self ):
        # Widget
        self.ww = 1
    ...
    # Grid
    def Grid_Size( self, value, grid_automatic, x, y ):
        self.grid_size = value
        self.grid_automatic = grid_automatic
        self.grid_x = x
        self.grid_y = y
        self.Render_Matrix()
        self.update()
    ...
    def mouseReleaseEvent( self, event ):
        ...
        Cursor_Icon( self, execption )
        if self.operation == "color_picker":
            ColorPicker_Event( self, self.ex, self.ey, self.qimage_grab, False, execption )
        # Variables
        self.operation = None
        # Update
        self.update()
    ...
    def wheelEvent( self, event ):
        ...
        angle = 5
        if delta_y >= angle:
            self.Grid_Increment( +1 * self.inverse_direction )
        if delta_y <= -angle:
            self.Grid_Increment( -1 * self.inverse_direction )
    # Drag and Drop Event
    def dragEnterEvent( self, event ):
...
class imagemanager_Reference( QWidget ):
    # General
    SIGNAL_DRAG = QtCore.pyqtSignal( [ str, dict ] )
    SIGNAL_DROP = QtCore.pyqtSignal( list )
    # Reference
    ...
    # Init
    def __init__( self, parent ):
        super( imagemanager_Reference, self ).__init__( parent )
        self.Variables()
    def Variables( self ):
        # Widget
        self.ww = 1
...
class Worker_Packer( QObject ):
    ...
    def run( self, source, mode, method ):
        ...
        # Print
        try:QtCore.qDebug( f"Image Manager | PACK { time.toString( 'hh:mm:ss.zzz' ) } | COUNT { count } | METHOD { method } | EFFICIENCY { efficiency } %" )
        except:pass
    ...
    def Pack_Linear( self, source, mode, method, pack_sort, pack_other, pin_list ):
        ...
        for s in range( 0, len( pack_sort ) ):
            ...
            if self.stop == True:
                ...
                loop = QMessageBox.question( QWidget(), "Image Manager", message, QMessageBox.Yes, QMessageBox.Abort )
        ...
        for s in range( 0, count ):
            ...
            if s == 0:
                ...
                if len( control ) > 0:
                    ...
                    self.stop = True
                    QMessageBox.information( QWidget(), i18n( "Warnning" ), i18n( "Image Manager | ERROR packer cycle" ) )
